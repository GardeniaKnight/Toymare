...
public class PlayerShooting : MonoBehaviourPun
{
    ...
    public GameObject bulletPrefab;      // 放在 Resources里
    public Transform bulletSpawnAnchor;
    ...
    void Update()
    {
        ...
        // 计算子弹颜色
        if (bounce && piercing)      bulletColor = bulletColors[3];
        else if (piercing)           bulletColor = bulletColors[2];
        else if (bounce)             bulletColor = bulletColors[1];
        else                         bulletColor = bulletColors[0];

        // 更新 UI
        if (bounceImage)   { bounceImage.gameObject.SetActive(bounce);   bounceImage.color = bulletColor; }
        if (pierceImage)   { pierceImage.gameObject.SetActive(piercing); pierceImage.color = bulletColor; }

        // 粒子与灯光
        if (gunParticles)  gunParticles.startColor = bulletColor;
        if (gunLight)      gunLight.color = bulletColor;
    ...
    void Shoot()
    {
        ...
        // 光效
        if (gunLight) { gunLight.intensity = 1 + 0.5f * (numberOfBullets - 1); gunLight.enabled = true; }

        // 粒子
        if (gunParticles) { gunParticles.Stop(); gunParticles.startSize = 1 + 0.1f * (numberOfBullets - 1); gunParticles.Play(); }
        ...
        for (int i = 0; i < numberOfBullets; i++)
        {
            ...
            // Photon 实例化：资源名必须在 Resources 文件夹下
            ...
            if (bs != null) { bs.piercing = piercing; bs.bounce = bounce; bs.bulletColor = bulletColor; }
        }
    }
